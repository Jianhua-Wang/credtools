...
def run_abf_cojo(
    ...
    logger.info("Step 1: Running COJO conditional selection to detect independent signals")
    ...
    if n_signals == 0:
        ...
        abf_result = run_abf(locus, max_causal=1, coverage=coverage, var_prior=var_prior)
        ...
        logger.info(f"Multiple signals detected ({n_signals}), running conditional ABF analysis")
...
def _run_conditional_abf_analysis(
    ...
    for i, signal_snp in enumerate(independent_snps):
        ...
        if len(conditioning_snps) > 0:
            ...
            try:
                ...
                logger.debug(f"Running conditional analysis conditioning on: {conditioning_snps}")
                ...
                conditional_locus = _create_conditional_locus(locus, conditional_results, signal_snp)
    ...
    logger.info(f"ABF+COJO analysis complete: {n_cs} credible sets from {n_signals} signals")
...
def _create_conditional_locus(
    ...
    conditional_map = conditional_results.set_index("SNP")[["cond_beta", "cond_se", "cond_p"]].to_dict()
    
    # Update the summary statistics
    for idx, row in original_sumstats.iterrows():
        snpid = row[ColName.SNPID]
        if snpid in conditional_map["cond_beta"]:
            original_sumstats.loc[idx, ColName.BETA] = conditional_map["cond_beta"][snpid]
