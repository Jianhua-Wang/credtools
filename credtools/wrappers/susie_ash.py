"""Wrapper for SuSiE-ash fine-mapping (SuSiE 2.0 with adaptive shrinkage background)."""
...
logger = logging.getLogger("SUSIE_ASH")

_R_SCRIPT_PATH = str(Path(__file__).parent / "susie_ash_wrapper.R")


def _check_r_and_susie_ash() -> None:
    ...
    if result.returncode != 0:
        raise FileNotFoundError(
            ...
            "SuSiE-ash requires susieR >= 0.16.1 (the SuSiE 2.0 release)."
        )


def _read_status(temp_dir: str) -> dict:
    """Parse the status file written by the R wrapper."""
    path = Path(temp_dir) / "susie_ash_status.txt"
    status: dict = {}
    if not path.exists():
        return status
...
def run_susie_ash(
    ...
    """
    Run SuSiE-ash fine-mapping with summary statistics and an LD matrix.

    SuSiE-ash extends SuSiE by adding an adaptive-shrinkage (ASH) prior over
    the residual / "unmappable" effects, a mixture of normals that captures
    a polygenic background of weak-to-moderate effects on top of the L
    sparse single-effect components. It is well suited to loci with one or
    a few strong signals sitting on top of many weak background effects.
    ...
    empty_on_nonconvergence : bool, optional
        When True and SuSiE-ash did not converge, return an empty credible
    ...
    CredibleSet
        ...
        and SuSiE-ash convergence metadata.
    ...
    RuntimeError
        If the SuSiE-ash R script execution fails.

    Notes
    -----
    The R wrapper invokes ``susieR::susie_rss(..., unmappable_effects="ash")``
    ...
    logger.info(f"Running SuSiE-ash on {locus}")
    ...
    with tempfile.TemporaryDirectory(prefix="susie_ash_") as temp_dir:
        ...
        logger.info(f"Running SuSiE-ash R script: {' '.join(cmd)}")
        result = subprocess.run(cmd, capture_output=True, text=True)

        if result.returncode != 0:
            logger.error(f"SuSiE-ash R script stderr: {result.stderr}")
            logger.error(f"SuSiE-ash R script stdout: {result.stdout}")
            raise RuntimeError(
                f"SuSiE-ash R script failed (return code {result.returncode}).\n"
                f"stderr: {result.stderr}\n"
                f"stdout: {result.stdout}"
            )
        logger.debug(f"SuSiE-ash R script stdout: {result.stdout}")

        pip_df = pd.read_csv(f"{temp_dir}/susie_ash_pips.csv")
        cs_df = pd.read_csv(f"{temp_dir}/susie_ash_cs.csv")
    ...
    if converged is False and empty_on_nonconvergence:
        logger.error(
            "SuSiE-ash did not converge in %d iterations; returning empty "
    ...
    if len(cs_df) > 0:
        ...
        logger.warning("SuSiE-ash found no credible sets.")
    ...
    logger.info(f"Finished SuSiE-ash on {locus}")
