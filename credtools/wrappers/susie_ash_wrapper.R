#!/usr/bin/env Rscript
# SuSiE-ash wrapper script for credtools.
# Called by credtools/wrappers/susie_ash.py via subprocess.
#
...
#   susie_ash_pips.csv   - SNP, PIP
#   susie_ash_cs.csv     - CS_ID, SNP
#   susie_ash_status.txt - tab-separated key/value with `converged` and `niter`
...
# SuSiE-ash is incompatible with susieR's default `EM` prior estimator;
# the paper requires `optim`, which we hardcode here.
estimate_prior_method      <- "optim"

cat("SuSiE-ash wrapper: starting\n")
cat(sprintf("  temp_dir: %s\n", temp_dir))
cat(sprintf("  n: %d\n", n))
cat(sprintf("  L: %d\n", L))
cat(sprintf("  coverage: %f\n", coverage))
cat(sprintf("  max_iter: %d\n", max_iter))
cat(sprintf("  estimate_prior_method: %s\n", estimate_prior_method))

if (!requireNamespace("susieR", quietly = TRUE)) {
  stop("susieR R package is not installed. Please install it with:\n",
       "  install.packages('susieR')\n",
       "Or for the development version with SuSiE-ash:\n",
...
cat(sprintf("Running susie_rss(unmappable_effects='ash') with %d SNPs...\n",
            n_snps))
...
write.csv(pip_df, file.path(temp_dir, "susie_ash_pips.csv"), row.names = FALSE)
...
write.csv(cs_rows, file.path(temp_dir, "susie_ash_cs.csv"), row.names = FALSE)

converged <- isTRUE(fit$converged)
niter_val <- if (!is.null(fit$niter)) as.integer(fit$niter) else NA_integer_
status_path <- file.path(temp_dir, "susie_ash_status.txt")
writeLines(
  c(
...
cat(sprintf(
  "SuSiE-ash wrapper: done. %d credible sets, converged=%s, niter=%s.\n",
